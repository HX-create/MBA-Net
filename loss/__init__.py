...
class Loss(nn.modules.loss._Loss):
    def __init__(self, args, ckpt):
        ...
        for loss in args.loss.split('+'):
            ...
            if loss_type == 'TripletLoss':
                ...
                loss_function = losses.ArcFaceLoss(num_classes=self.args.num_classes, embedding_size=2048)
            else:
                raise RuntimeError
    ...
    def forward(self, outputs, labels):
        ...
        for i, l in enumerate(self.loss):
            if l['type'].find('CrossEntropy') is not -1: # 检索到了
                loss = l['function'](outputs[0], labels)  # score
                effective_loss = l['weight'] * loss
                losses.append(effective_loss)
                self.log[-1, i] += effective_loss.item()
            elif l['function'] is not None:
                loss = l['function'](outputs[1], labels)  # feature
                if l['type'] == 'ArcFaceLoss':
                    loss = loss / (self.args.batchid * self.args.batchimage)
                effective_loss = l['weight'] * loss
                losses.append(effective_loss)
                self.log[-1, i] += effective_loss.item()
            else:
